...
## 14. Простой слог и живой ритм

Сложность идёт не от длины предложения, а от того, как оно построено. Ломают
текст четыре вещи.
...
**Афоризм разворачивать.** «Дефицит переехал», «стена freemium оказалась
роскошью» — красиво и требует работы от читателя. Пишем прямо: «код писать стало
дёшево, а находить вас — нет».

**Не отсылать числами.** «Вторую пропускают чаще всего», «оба варианта плохие»
заставляют держать в голове, что было первым. Назвать вещь ещё раз дешевле.

**Но не рубить.** Первая версия этого правила требовала коротких фраз, сайт
переписали, и владелец назвал результат рваниной — справедливо. Замер по 33 его
черновикам: медиана 10 слов, отклонение 10.8, десятый перцентиль 4 слова,
девяностый 28, каждое восьмое предложение длиннее 24 слов. То есть короткая
фраза стоит рядом с длинной, и это и есть речь. Ровный текст из фраз одной
длины читается хуже неровного, даже когда каждая фраза по отдельности простая.

Поэтому длинное предложение — не грех. Грех — когда все предложения одинаковые.
Связывайте соседние мысли союзами, где они правда связаны: «потому что»,
«чтобы», «зато», «а». Разрывайте только там, где связи нет.

Линтер проверяет три вещи, и все три — предупреждения, а не ошибки: отдельное
предложение не длиннее 36 слов, медиана по странице не больше 14, разброс длин
не меньше 7. Последнее и ловит рванину.

Образец, по которому равняться: `src/content/docs/ru/demand/pick-a-niche.mdx`.

## 15. Слова-костыли

Голос проверяется не только формой предложения, но и тем, за какие обороты
рука тянется по умолчанию. Замер 2026-08-17 по 33 черновикам владельца против
русской версии сайта, вхождений на тысячу слов:

| Оборот | У владельца | Было на сайте |
|---|---|---|
| «X, а не Y» | 2.3 | 7.8 |
| «поэтому» | 0.5 | 2.2 |
| «значит» | 0.4 | 1.6 |
...
Противопоставление «X, а не Y» — главный тик. Само по себе оно нормальное, и
там, где контраст и есть мысль («лимит ключа показывает потолок трат, а не
баланс»), его надо оставлять. Плохо, когда им написана каждая третья фраза:
страница превращается в список антитез и начинает звучать заученно.

Лечится просто. Утверждайте прямо и не дописывайте, чем это не является.
«Считайте худший случай, а не средний» → «Считайте по худшему случаю».

Линтер предупреждает, если «а не» встречается чаще пяти раз на тысячу слов —
вдвое выше авторской нормы, так что ловит только заросшие страницы.
