...
Write the vocabulary down before the first link goes out: a fixed list of source values, one per venue, in a file somebody can open.

Tagging as you go does not work. Inside a month you have `reddit`, `Reddit`, `reddit-selfhosted` and `r/selfhosted`, and those four never merge cleanly afterwards, because you no longer remember which post went out with which tag.
...
It does, and this page used to claim otherwise. Telegram documents 64 characters of `A-Za-z0-9_-` in the start payload, and the link you hand out looks like this:
...
Put the raw parameter in the second column, exactly as it arrived. At some point you will change the scheme and want to re-read the history through the new scheme, and `source_raw` is the only thing that lets you.
...
- **Asking "how did you find us" on the first screen.** The question stands between the person and the value, and that is precisely where onboarding loses people. The answer is worth less than the extra step costs.
- **One tag for a whole launch day.** Five venues under one tag told me the launch worked. They did not tell me which of the five to do again, and that was the only thing I wanted to know.

## Verify

- Open your own tagged link, sign up through it, then read the row back out of the database, where the source value has to be sitting.
- Come back through a different tagged link. The stored value must be unchanged.
- Check the share of records carrying any source at all, once a week, and the more untagged links you fix, the higher that share should be.
  ...
  order by week desc;
  ```
- Compare self-reported answers against stored tags, and where an answer and a tag disagree, the tag is usually not wrong. It is simply missing.
- Send yourself a link with a tag your parser has never seen, and it must land in `unknown` — not crash, and not quietly become direct.
