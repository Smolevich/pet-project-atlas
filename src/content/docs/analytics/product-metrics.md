...
"They did not convert" and "the button was below the fold on a phone" produce the same row in most tables. Not converting is a verdict on demand. A button below the fold is an afternoon of work, and you cannot tell them apart later if the row does not know the difference now.
...
The row decides what you can ask later. The panel that reads the row decides nothing. A panel that turns out to be wrong is an evening of work, but a field nobody wrote is missing for the whole period nobody wrote it. There is no way back.
...
`user_id` is a stable internal id rather than a session, because sessions end and the question you will ask later is about a person. `name` comes from a closed list in code. `outcome` is `ok` or a named error class, never a boolean: a boolean throws away the only part of a failure you can act on.

### What has to be written at insert time, because no join brings it back

Cost and source are the two that catch everybody. You know the cost of an action only while the provider's response is still in your hand, and the details are on [what one user costs](/money/unit-economics/). You know the source only at first contact, and that is [where the user actually came from](/analytics/attribution/).
...
There is a third gap on the same table, and it dates precisely. A `meta` field was added on 31 May, so the 219 rows written between 13 and 30 May have it empty. That is where the kind of input lives — voice message, uploaded file, video — so there are eighteen days of arrivals I can never sort by what people actually sent.
...
Keep the event names in a closed list in code, not free text at the call site. Free text gives you four spellings of one step that nothing merges afterwards, and the hand-written mapping you build to paper over it rots quietly at the next rename.

### How to read failures so they say something about the product

Log errors with the same identity as events: user id, the action, the class of input. An error line without a user id answers an operations question, and that is a fine thing for a line to do at three in the morning. The same line with a user id answers a product question.
...
- **Building the panel before writing the questions.** I read panels that answered nothing, week after week. I kept it up for longer than I want to write down, and in the end I deleted most of my event tables and rebuilt the thing around cost and exhaustion.
- **Never adding a cost column at all.** I thought I had left it empty. I opened the table for this page — there is no such column and never was, across all 1,180 rows. Every cost I have quoted I reconstructed later from a rates table, and on a chart a reconstruction and a measurement are the same line.
...
- **Not recording the kind of input.** The field arrived on 31 May, 18 days into the bot's life. The 219 rows before it stay blank forever. Since it exists, uploaded files rival speech into the chat — 256 against 341 — and every first screen I had written before that spoke to the other person.
