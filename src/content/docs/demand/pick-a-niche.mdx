...
Fifty reviews to a page, around nine pages, then nothing. Every review carries a date, a rating and the app version, and the version tells you which release the complaints started on. You change the country right in the address — `gb`, `fr` and `de` complain about different things.
...
Before 17 July the starting grant was not credited to new users, so somebody arrived, had zero credits, and could do nothing at all. That was my bug, not the market.
