...
A bank's developer docs, somebody's listicle, a tech-media post, a CRM's help centre. Not one of them is a product like mine. The listicle in second place is the kind of page I could be inside of.

## Steps

Sort the venues before you touch any of them. Each one costs you the same hour and buys you something different.
...
One card written in advance keeps all the entries consistent. Consistent entries are what let scattered mentions merge back into one entity later.

Put a tracking parameter on every link you submit, one value per venue. Write it into the card rather than into the form.
...
Lists of "best directories" go stale faster than anything else in this field. Check each venue yourself, on the day you submit, instead of trusting the list.
...
Then open one listing with JavaScript off. A card rendered client-side is an empty page to a crawler. One request tells you which kind you are dealing with:
...
- **Mass submission without a prepared card**. Every form got a slightly different description and a different category. The entries scattered, and nothing could merge them back into one entity.
...
- **Submitting untagged links**. The listings went live and no signup could be traced to any of them. Whether the venues worked at all, I had no way to tell.
...
- **Chasing volume in directories**. Bulk entries in low-quality catalogs are a documented spam pattern. They also eat the week you needed for one outreach email.
...
- After a few weeks, ask an assistant for tools in your category, several runs each. Directories move exactly this: whether it names you at all.
