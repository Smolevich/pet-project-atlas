...
Your product lives inside Telegram, an app store or a marketplace, which means that platform runs its own search over its own index, and your website has no influence on that index whatsoever.
...
Put the function in the name, not only the brand, and write it in the script your audience types in, because everything else on the profile is read after somebody has found you.
...
Apple's keyword field holds 100 bytes, comma-separated. Bytes, not characters. ASCII spends one byte a letter and Cyrillic in UTF-8 spends two, so for a Russian list that is about 50 letters.
...
Ask the platform what is actually set, because your memory is not evidence here and neither is the console you typed the values into.
