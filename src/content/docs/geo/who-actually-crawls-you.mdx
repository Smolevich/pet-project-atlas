...
5. **Count the agents over the whole window** — one match per request, case-insensitive.
   ...
   The `awk` matters here, rather than the familiar `grep -oE` this page once shipped. `grep` prints occurrences instead of lines, and a single bingbot request names itself twice — in the token and in the `+http://…/bingbot.htm` URL. In this window that comes out as 336 occurrences against 155 requests, so the count doubles. `awk` takes the first match per line, and one line means one request again.
