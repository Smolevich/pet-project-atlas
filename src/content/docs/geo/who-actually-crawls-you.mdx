...
- **Counting my own traffic as an agent's**. My address is in this log 4,221 times, 81 of them under a crawler name, and I published the totals with all of it inside. I blamed the strange rows on an unknown scanner. It was my own `curl` loop, and one call to `api.ipify.org` said so.
