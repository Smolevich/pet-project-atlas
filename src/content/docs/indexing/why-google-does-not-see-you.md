...
- **Checking only from my own laptop**. A logged-in session, a warm service worker and a home network the edge already trusts will hide the failure.
- **Editing `robots.txt` when the block lived at the edge**. Bot-protection and WAF rules are invisible in that file, and the file was clean the whole time. This is the most common hidden blocker I run into.
