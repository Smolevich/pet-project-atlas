...
Этот payload занимает 28 символов. В нём уже лежат площадка, канал и дата, и место ещё осталось. Разделителей вам дают два: дефис и подчёркивание, поэтому выберите один, запишите выбор в тот же словарь и больше не мешайте дефисы с подчёркиваниями. Парсер, которому приходится угадывать, рано или поздно угадает неверно.
...
Весь смысл здесь в `do nothing`. Человек вернулся с другой меткой, попал в конфликт и исходное значение не тронул, и это то, что нужно. Last touch почти всегда оказывается вашей же ссылкой, потому что вернувшиеся приходят через ваш канал, и они перезаписывают ровно тот ответ, ради которого всё затевалось.
