...
Вот моя собственная таблица, которая проваливает и стоимость, и источник, замер 13 августа 2026-го. В `usage_events` 1180 строк начиная с 13 мая. Колонок, в имени которых есть `cost` или `price`, — ноль. Колонок с источником — ноль.
...
Есть и третий пробел на той же таблице, и он датируется точно. Поле `meta` появилось 31 мая, поэтому у 219 строк, написанных с 13 по 30 мая, оно пустое. А именно там лежит вид входа — голосовое, залитый файл, видео, — и восемнадцать суток приходов я уже никогда не разложу по тому, что человек прислал.
...
У меня оно делится почти пополам. Из 76 строк со статусом `error` 44 — это то, что сломалось: отвалившийся шлюз, слишком большой файл, упавший перекодировщик. Остальные 32 — бот сообщает человеку, что бесплатный лимит кончился, и это решение о тарифах в костюме ошибки. В эту стену упёрлись шесть разных людей.
...
- **Не записывать, какого вида приходит вход.** Поле появилось 31 мая, на восемнадцатые сутки жизни бота, и 219 строк до него так и останутся пустыми. С тех пор видно, что залитые файлы спорят с наговорённым в чат: 256 против 341, а все первые экраны, которые я до этого написал, обращались к другому человеку.
