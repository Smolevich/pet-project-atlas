...
Под `robots.txt` лежат ещё три слоя: заголовок `X-Robots-Tag`, мета-тег и edge. В самом файле не виден ни один из них, так что отвечать 403 может переключатель в CDN, правило WAF или rate limit, пока вы правите файл, который никто не читает. WAF — это firewall, который матчит HTTP-запросы.
