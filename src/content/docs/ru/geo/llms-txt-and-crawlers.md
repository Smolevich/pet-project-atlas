...
Кому разрешено читать сайт, решаете вы. На большинстве проектов это решение принимают случайно, в панели, которую никто не открывал. Если crawler до вас не дошёл, в AI-ответах вас нет. Насколько хорошо написана страница, после этого уже неважно.

Ловушка сидит в самих словах «AI-бот». За ними прячутся три разные работы. Отказать в доступе каждой из трёх стоит по-разному.
...
Читайте то, что отдаёт боевой домен, а не то, что лежит в репозитории. CDN умеет переписать этот файл по дороге.

```bash
curl -sS https://example.com/robots.txt
```

Managed-версия Cloudflare живёт на странице **Security Settings** зоны, под фильтром **Bot traffic**. Она есть на всех тарифах. Если её включить, она допишет свои строки `Disallow` перед вашими. Домену на Free-тарифе без собственного файла Cloudflare отдаёт вместо него свою Content Signals Policy.
...
В Google Search его блокировка не меняет ничего. AI Overviews — часть Search. Страницы для них обходит обычный `Googlebot`.

### Как выйти из AI Overviews по-настоящему

Для этого есть отдельный контрол: Search Console, Settings, Search generative AI. Он покрывает AI Overviews, AI Mode и генеративные блоки в Discover. Google пишет, что сигналом ранжирования в остальном Search он не служит.

`nosnippet`, `data-nosnippet` и `max-snippet` тоже выводят. Только они забирают с собой и обычный сниппет в выдаче. Отдельный контрол такой цены не берёт. Раскатывают его пока на часть владельцев, так что у вас в ресурсе его может ещё не быть.
...
Под `robots.txt` лежат ещё три слоя: заголовок `X-Robots-Tag`, мета-тег и edge. В самом файле не виден ни один из них. Отвечать 403 может переключатель в CDN, правило WAF или rate limit. WAF — это firewall, который матчит HTTP-запросы. А вы всё это время правите файл, который никто не читает.

В Cloudflare этот контрол называется **Configure AI bot policies** и лежит на странице **Security Settings** зоны. Он есть на всех тарифах, включая Free. Агентов он раскладывает на Search, Agent и Training. Для каждой группы доступно Allow, Block on all pages или Block only on pages with ads.
...
С 15 сентября 2026 новый для Cloudflare домен приезжает с блоком Training и Agent на страницах с рекламой. Старый одиночный переключатель рядом, **Block AI bots**, в тот же день уходит. Существующие зоны сохраняют свои настройки. Отказаться от новых умолчаний можно до 15 сентября.
...
Строку user agent может отправить кто угодно. Я и отправил: мой собственный цикл на `curl` положил в access-лог имена вендоров. Две недели я читал эти строки как визиты. Вендоры публикуют диапазоны IP и обратный DNS для своих crawler. Сверяйтесь с ними, прежде чем считать строку лога доказательством визита или атаки. Чего мне это стоило, разобрано в [кто к вам на самом деле ходит](/ru/geo/who-actually-crawls-you/).

### Стоит ли писать llms.txt

Если вы отдаёте документацию для разработчиков — да. В остальных случаях скорее нет. Дело тут не в моём отношении к формату.

Файл стоит десять минут. Читать его никто из вендоров не обязался. Что было у меня за 16 дней:
...
Две оговорки, без которых этот ноль читать нельзя. Окно дырявое: logrotate держит 14 суточных архивов, и записи с 20 по 28 июля не сохранились. И ещё: запрос, зарезанный на edge, до nginx не доходит. В таблицу выше он не попадает вообще. Со стороны сервера этого не увидеть никак. Поэтому я сходил и сверил обе стороны за одни сутки:
...
Числа сходятся. Ни одного 403 нет ни там, ни там, то есть мой edge не режет никого. Разница в четыре запроса — это окно: на стороне nginx оно на час шире. Точнее свести не дал бесплатный тариф Cloudflare. Он отдаёт аналитику окном не больше суток.

Проверять надо именно так. Если бы edge резал, счётчик Cloudflare был бы заметно больше серверного. Эта разница и была бы ответом. Разбор целиком: [кто к вам на самом деле ходит](/ru/geo/who-actually-crawls-you/).

То есть на маленьком сайте про продукт вы ставите десять минут против измеренной отдачи 0. В документации на сорок страниц ставка разумная. В остальных случаях я бы эти десять минут потратил на `robots.txt` и sitemap. Их забирают все.
...
Поддержка `llms.txt` — договорённость, а не стандарт, соблюдать который кого-то заставляют. Честный ответ про своих гостей лежит в access-логе.
...
Запустите `geo-crawlers` из раздела [Tools](/ru/tools/). Он читает `robots.txt`, мета-теги и заголовки ответа и отдаёт карту доступа по агентам. Так и находится правило, о котором вы забыли.

Оттуда же запустите `geo-llmstxt`. Он проверяет существующий файл или собирает черновик по структуре сайта. Ссылки, ведущие в никуда, он помечает.
...
- Сначала выясните, что вообще стоит перед вашим сервером.
  ...
  Заголовок `cf-ray` означает Cloudflare. Vercel, Netlify и Fastly представляются своими заголовками. Если впереди нет ничего, отказать crawler'у могут только файрвол и веб-сервер. Проверки ниже тогда переезжают туда.
