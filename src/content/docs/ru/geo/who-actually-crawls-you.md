...
В окне дыра. Ничего между 20 и 28 июля ротацию не пережило. Дальше речь про 16 дней записей.

Это трафик одного оператора, а не выборка по вебу. Зато это замер. У советов вокруг нет и такого.

Первым замер нашёл меня. Мой собственный адрес сделал за окно 4 221 запрос к этой машине. 81 запрос из них ушёл под именем краулера.

## Шаги

Сначала прочитайте весь период целиком, а отдельного агента — потом. Запросы с моего собственного адреса стоят в столбце «Мои».
...
Счёт был почти верный. Клиентом был я. С этого адреса за окно пришло 56 разных строк `User-Agent`, и 18 из них — имена краулеров. Все они пришли из `curl`, которым я проверял собственные правила доступа.
...
То есть строка была настоящая, а краулера за ней не было. `Google-Extended` вы не выгрепаете из своего лога никогда. Разве что наберёте его сами себе.
...
Четыре запроса из 341 пришли с моего адреса. Это был `curl` с руками выставленной строкой. Остальные 337 пришли с 272 адресов. С опубликованными диапазонами вендоров я эти адреса не сверял.
...
Всю строку `ChatGPT-User` надо читать так. Это счёт обращений агента, которого обязан запустить человек. Кто был этим человеком, лог не говорит.
...
Прошлая версия страницы писала «примерно по одному запросу на агента» и называла файл оглавлением, в которое заглядывают. Это чтение оказалось одним моим циклом.
...
Так что `llms.txt` — не маршрут, по которому ходят агенты. На этой машине в это окно он не был даже оглавлением. Направление вывода я оставил, а цифру пришлось убрать.
...
5. **Посчитать агентов за всё окно** — по одному совпадению на запрос, без учёта регистра.
   ...
   Очевидный вариант через `grep -oE` неверен дважды, и эта страница его публиковала. Bing зовёт себя строчными `bingbot`. Регистрозависимый шаблон не находит его вовсе.
   Почините регистр — вылезет вторая ошибка. `grep -o` печатает вхождения, а не строки. Запрос от bingbot называет себя дважды: в токене и в URL `+http://…/bingbot.htm`.
...
Сравнение, ради которого всё и затевалось: 0 обращений к `llms.txt` против 56 к `robots.txt`. `robots.txt` взял каждый AI-агент из таблицы. `llms.txt` не взял ни один. Это не значит «файл бесполезен». Это значит ровно то, что подтверждает лог одной машины за 16 дней.

К `/` пришло 280 запросов, больше, чем ко всем остальным путям вместе. Прочитать это как предпочтение я не могу. Голый домен, вставленный в чат, и агент, решивший начать с корня, дают одну и ту же строку. Лог записывает путь и никогда не записывает причину. Держать хорошие страницы достижимыми от корня — по-прежнему здравый совет. Только доказывает его не эта таблица.

Пост со сравнением забрали 64 раза, и ещё 6 раз по-русски. Это больше, чем весь остальной блог вместе, и это всё, что знает про него nginx. Ответ 200 значит, что байты ушли. Попало ли из них хоть слово в чужой ответ, лог не говорит. Цитирование видно в переходах с хостов ассистентов и в самих ответах. Ни то ни другое в лог не пишется.

`robots.txt` забрали 56 раз, `sitemap.xml` — 14 раз. Эти две строки я читаю как протокол, а не как интерес ко мне. Crawler берёт `robots.txt` в начале каждой сессии. Большое число здесь значит, что рукопожатие работает. Про правильный sitemap никто не пишет постов. А агенты всё равно просят его раньше любой вашей прозы.
