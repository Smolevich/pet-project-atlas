...
И считайте худший случай, а не средний. Пользователь имеет полное право спустить весь пакет на самый дорогой движок. Нижняя граница маржи — минимум по пакетам, посчитанный именно на этом движке, и приемлемым должно быть именно это число, а не среднее по всем.
...
Свои аккаунты вычитайте явным списком id, и вычитайте первым действием, до любой агрегации. Считайте при этом уникальных платящих, а не транзакции, потому что несколько покупок одного человека — это один платящий, а не три.
...
Дефолтом ставьте самый дешёвый движок, который проходит по качеству. Дефолт получают почти все. Ваши настоящие расходы описывает путь по умолчанию, а не список опций в настройках, и дорогой движок должен жить за осознанным выбором.
