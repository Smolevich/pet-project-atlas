...
Вопрос закрывают два числа: переменная стоимость одного действия и фиксированная стоимость месяца, в который не пришёл никто. Break-even считается из них, в платящих за месяц. Процент маржи не отвечает, продолжать или нет. «Два человека в месяц» — отвечает.
...
Стоимость вызова пишите в ту же строку события, что и результат, и сразу, потому что через час взять её будет уже неоткуда. Многие API отдают usage прямо в ответе, некоторые — списание в деньгах. Отдаёт только количество — храните количество и умножайте на тариф.
...
Смотрите на разницу между `actions` и `priced`. У меня такой колонки не было вовсе, и стоимость я добирал джойном к тарифам задним числом. Тарифы меняются, джойн берёт сегодняшние, ответ про май тихо уезжает — а на графике добранное неотличимо от замеренного.
