...
- [ ] Fetch a page with `curl -sL` from outside, without cookies, and find your text in the response — [blockers](/indexing/why-google-does-not-see-you/)
...
- [ ] Add JSON-LD — machine-readable facts about the page — with `sameAs` for your profiles and listings. Documented for Google's Knowledge Graph, unmeasured for AI citation — [citability](/geo/citable-pages/)
...
- [ ] Record each action's cost on its event row, then state break-even in whole payers. My cost column was empty and every report fell back to estimated rates — [cost](/money/unit-economics/)
...
- **Starting at the indexing checks**. This list used to start there, and starting there quietly assumes the demand exists. Everything below the first block is aimed at words somebody has to be typing.
- **Starting in the middle, at the interesting part**. Content and directories are the enjoyable half, and both of them are wasted while the pages still carry a header that drops them out of the index.
- **Ticking the tagging item with nothing to store the tag**. My links went out instrumented, the parameter arrived on every visit, and there was no column waiting for it. I ticked the box honestly and the data was gone anyway.
- **Treating it as a list you finish once**. Directory status, edge rules and crawler access are all state, and state changes without telling you, so I re-run the top half each quarter.
- **Keeping an item this site's own practice contradicted**. The list told you to allow agents by name while `atlas.smolevich.com` was serving a plain wildcard the whole time. My own file was right and the list was wrong.
- **Requiring `llms.txt` after measuring that nothing fetched it**. The item sat here for exactly as long as the measurement did, one page away, and I kept ticking it.

## Verify

- Every ticked box has an artefact behind it: a command output, a report row, a row in the database. Memory does not count.
- The demand boxes leave a file of phrasings and a named competitor. If either one is missing, everything ticked below it is aimed at nothing.
- Check the tagging items by signing up through your own link and reading back the value that got stored.
